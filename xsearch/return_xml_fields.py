...
def get_elements_tags(
    ...
    for parent in parent_generator:
        ...
        if args.id:
            idn = "".join(
                p.text for p in parent.xpath(args.id, namespaces=namespaces)
            )
...
def main():
    ...
    aparse = argparse.ArgumentParser(
        description="Read through a directory \
            of xml files and return different elements and values of the xml"
    )
    aparse.add_argument(
        "-d",
        "--dir",
    ...
    aparse.add_argument(
        "-i", "--id", help="Set an Xpath to the parent id number"
    )
    aparse.add_argument(
        "-a",
        "--attrib",
    ...
    aparse.add_argument(
        ...
        "--tail",
        action="store_true",
        help="Return the element tail text",
    )
    aparse.add_argument(
        "-g", "--tag", action="store_true", help="Return the Element tag"
    )
    aparse.add_argument(
        "--padding", help="Set padding for the column rows", type=int
    )
    aparse.add_argument(
        "-p", "--parent", help="Parent tag to return parent element"
    )
    aparse.add_argument(
        "filter_xpath", help="The xpath query to filter the records on"
    )
    aparse.add_argument(
        "results_xpath",
        nargs="*",
        help="Find a set of values from the parent of the filter record, \
    ...
    element_list, namespaces = path_walker(
        directory, filter_xpath=args.filter_xpath
    )
    if args.parent:
    ...
    parent_generator = map(
        lambda x: get_parent_by_tag(x, parent_tag), element_list
    )
    results = get_elements_tags(
        parent_generator, args, args.results_xpath, namespaces
    )
    if args.padding:
